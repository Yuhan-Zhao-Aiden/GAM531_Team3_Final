...
public sealed class GameScene : IScene
{
  ...
  private double _walkSoundCooldown;
  ...
  public void Initialize(string contentRoot, Vector2i viewportSize)
  {
    ...
    _enemy.OnProjectileFired += projectile =>
    {
      ...
      projectile.OnExplode += () => _audioSystem?.PlaySound("explosion.mp3");
    };
    ...
    // Initialize Audio system and play BGM
    _audioSystem = new AudioSystem(contentRoot);
    _audioSystem.PlayBgm("bgm.mp3", loop: true);

    // Hook up sound events
    _player.OnAttackStart += () => _audioSystem?.PlaySound("swing.mp3");
    _enemy.OnShoot += () => _audioSystem?.PlaySound("shoot.mp3");
  ...
  public void Update(double deltaSeconds)
  {
    ...
    // Play walk sound when player is moving horizontally
    if (_player is not null && !_player.IsDead)
    {
      bool isMoving = Math.Abs(_player.Velocity.X) > 50f && _player.IsGrounded && !_player.IsAttacking && !_player.IsRolling;

      if (isMoving)
      {
        if (_walkSoundCooldown <= 0)
        {
          _audioSystem?.PlayRandomWalkSound();
          _walkSoundCooldown = 0.4; // Play walk sound every 0.4 seconds
        }
      }

      if (_walkSoundCooldown > 0)
      {
        _walkSoundCooldown -= dt;
      }
    }

    // Play game over or win music
    if (!_gameOverSoundPlayed)
    {
      if (_player is not null && _player.IsDead)
      {
        _audioSystem?.StopBgm();
        _audioSystem?.PlaySound("gameover.mp3");
        _gameOverSoundPlayed = true;
      }
      else if (_enemy is not null && _enemy.IsDead)
      {
        _audioSystem?.StopBgm();
        _audioSystem?.PlaySound("win.mp3");
        _gameOverSoundPlayed = true;
      }
    }
  }
